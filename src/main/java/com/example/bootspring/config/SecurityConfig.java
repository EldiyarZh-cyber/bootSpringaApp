...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http
                .csrf().disable()
                .authorizeRequests()
                .antMatchers(new String[]{"/blog-add","/blog"}).authenticated()
                .antMatchers("/blog-add").hasAuthority("ROLE_ADMIN") // .hasRole(ADMIN) // hasRole will automatically add prefix ROLE_
